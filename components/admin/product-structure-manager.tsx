...
function AddForm({ onAdd, onCancel, placeholder }: { onAdd: (name: string) => void; onCancel: () => void; placeholder: string }) {
...
// ── Generic sortable file row ─────────────────────────────────────────────────

function SortableFileRow({ dl, manufacturerId, productTypeId }: { dl: Download; manufacturerId: string; productTypeId: string }) {
  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }}
      ...
      <button {...listeners} {...attributes} className="text-brand-gray/30 hover:text-brand-gray cursor-grab active:cursor-grabbing shrink-0" aria-label="Verschieben">
      ...
      <Link href={`/admin/manufacturers/${manufacturerId}/products/${productTypeId}/downloads/${dl.id}`}
...
function SortableFilesList({
  ...
  const idsKey = initialFiles.map((d) => d.id).join(',')
  const [fileIds, setFileIds] = useState(() => initialFiles.map((d) => d.id))
  const fileIdsRef = useRef(fileIds)
  useEffect(() => { fileIdsRef.current = fileIds }, [fileIds])
  useEffect(() => { setFileIds(initialFiles.map((d) => d.id)) }, [idsKey])

  const fileMap = Object.fromEntries(initialFiles.map((d) => [d.id, d]))
  const initialIds = initialFiles.map((d) => d.id)
  ...
  return (
    <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
      <SortableContext items={fileIds} strategy={verticalListSortingStrategy}>
        {fileIds.map((id) => {
          const dl = fileMap[id]
          return dl ? <SortableFileRow key={id} dl={dl} manufacturerId={manufacturerId} productTypeId={productTypeId} /> : null
        })}
      </SortableContext>
    </DndContext>
  )
...
function GroupItem({ group, manufacturerId, productTypeId, onUpdated }: { group: GroupWithFiles; manufacturerId: string; productTypeId: string; onUpdated: () => void }) {
  ...
  return (
    <div className="border border-brand-light-gray bg-white">
      <div className="flex items-center gap-2 px-3 py-2.5 group/header">
        ...
        {editing ? (
          <InlineEditor value={group.name} onSave={handleEditSave} onCancel={() => setEditing(false)} placeholder="Gruppenname" />
        ) : (
          <>
            ...
            <div className="opacity-0 group-hover/header:opacity-100 flex items-center gap-2 shrink-0">
              <button onClick={() => setEditing(true)} className="text-brand-blue hover:text-brand-dark-blue"><Pencil size={12} /></button>
              <button onClick={handleDelete} className="text-red-400 hover:text-red-600"><Trash2 size={12} /></button>
            </div>
          </>
        )}
      </div>
      ...
      {expanded && (
        <div className="border-t border-brand-light-gray/60">
          {group.downloads.length > 0 ? (
            <SortableFilesList files={group.downloads} manufacturerId={manufacturerId} productTypeId={productTypeId} onUpdated={onUpdated} />
          ) : (
          ...
          <div className="px-3 py-2 border-t border-brand-light-gray/60">
            <Link href={`/admin/manufacturers/${manufacturerId}/products/${productTypeId}/downloads/new?groupId=${group.id}`}
...
function SortableGroupItem({ group, manufacturerId, productTypeId, onUpdated }: { group: GroupWithFiles; manufacturerId: string; productTypeId: string; onUpdated: () => void }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: group.id })
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }}
      className={`flex items-start gap-1 touch-none ${isDragging ? 'opacity-30' : ''}`}>
      <button {...listeners} {...attributes} className="mt-2.5 text-brand-gray/30 hover:text-brand-gray cursor-grab active:cursor-grabbing shrink-0" aria-label="Gruppe verschieben">
        <GripVertical size={14} />
      </button>
      <div className="flex-1 min-w-0">
        <GroupItem group={group} manufacturerId={manufacturerId} productTypeId={productTypeId} onUpdated={onUpdated} />
      </div>
    </div>
  )
}
...
function GroupsList({ groups: initialGroups, sectionId, productTypeId, manufacturerId, onUpdated }: { groups: GroupWithFiles[]; sectionId: string | null; productTypeId: string; manufacturerId: string; onUpdated: () => void }) {
      ...
      <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
        <SortableContext items={groupIds} strategy={verticalListSortingStrategy}>
          {groupIds.map((id) => {
            ...
            return group ? <SortableGroupItem key={id} group={group} manufacturerId={manufacturerId} productTypeId={productTypeId} onUpdated={onUpdated} /> : null
          })}
        </SortableContext>
      </DndContext>
      {adding ? (
        ...
        <button onClick={() => setAdding(true)} className="flex items-center gap-1 text-xs text-brand-blue hover:text-brand-dark-blue font-bold mt-1">
...
function SortableSectionItem({ section, productTypeId, manufacturerId, onUpdated }: { section: SectionWithContent; productTypeId: string; manufacturerId: string; onUpdated: () => void }) {
  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }}
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 bg-brand-dark-blue/5 group/sec">
        <button {...listeners} {...attributes} className="text-brand-gray/30 hover:text-brand-gray cursor-grab active:cursor-grabbing touch-none shrink-0" aria-label="Bereich verschieben">
          <GripVertical size={14} />
        </button>
        <button onClick={() => setExpanded((e) => !e)} className="text-brand-gray/50 hover:text-brand-gray shrink-0">
          {expanded ? <ChevronDown size={13} /> : <ChevronRight size={13} />}
        ...
        {editing ? (
          <InlineEditor value={section.name} onSave={handleEditSave} onCancel={() => setEditing(false)} placeholder="Bereichsname" />
        ) : (
          <>
            <Layers size={13} className="text-brand-dark-blue shrink-0" />
            <span className="flex-1 text-sm font-bold text-brand-dark-blue uppercase tracking-wide min-w-0 truncate">{section.name}</span>
            <span className="text-xs text-brand-gray/50 shrink-0">
              {section.groups.length}G · {section.directFiles.length}D
            </span>
      ...
      {expanded && (
        <div className="p-3 bg-brand-surface/40 space-y-3">
          ...
          {/* Direct files within section */}
          <div>
            <p className="text-xs text-brand-gray/60 mb-1">Direkte Dateien im Bereich:</p>
            {section.directFiles.length > 0 && (
              <div className="border border-brand-light-gray">
                <SortableFilesList
                  files={section.directFiles}
                  manufacturerId={manufacturerId}
                  productTypeId={productTypeId}
                  onUpdated={onUpdated}
                />
            ...
            <Link
              href={`/admin/manufacturers/${manufacturerId}/products/${productTypeId}/downloads/new?sectionId=${section.id}`}
              className="inline-flex items-center gap-1 text-xs text-brand-blue hover:text-brand-dark-blue font-bold mt-1"
            >
              <Plus size={11} /> Datei direkt hinzufügen
            </Link>
          </div>
        </div>
      )}
...
function SortableStandaloneRow({ dl, manufacturerId, productTypeId }: { dl: Download; manufacturerId: string; productTypeId: string }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: dl.id })
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }}
...
export function ProductStructureManager({ sections: initialSections, looseGroups: initialLooseGroups, standaloneFiles: initialStandaloneFiles, productTypeId, manufacturerId }: Props) {
  const router = useRouter()
  function handleUpdated() { router.refresh() }
  ...
  const totalFiles =
    initialSections.reduce((acc, s) => acc + s.groups.reduce((a, g) => a + g.downloads.length, 0) + s.directFiles.length, 0) +
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        {/* Allgemeine Dateien (standalone) — ganz oben */}
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs font-bold text-brand-gray uppercase tracking-wider">Allgemeine Dateien</p>
            <Link href={`/admin/manufacturers/${manufacturerId}/products/${productTypeId}/downloads/new`}
            ...
            <SortableContext items={standaloneIds} strategy={verticalListSortingStrategy}>
              <div className="space-y-1">
                {standaloneIds.map((id) => {
                  ...
                  return dl ? <SortableStandaloneRow key={id} dl={dl} manufacturerId={manufacturerId} productTypeId={productTypeId} /> : null
                })}
              </div>
            </SortableContext>
          </DndContext>
          {standaloneIds.length === 0 && (
            <div className="border-2 border-dashed border-brand-light-gray py-4 text-center">
              <p className="text-xs text-brand-gray/60">Keine allgemeinen Dateien.</p>
            </div>
          )}
        </div>
        ...
        <div className="mb-6">
            ...
            <SortableContext items={sectionIds} strategy={verticalListSortingStrategy}>
              <div className="space-y-2">
                {sectionIds.map((id) => {
                  ...
                  return section ? (
                    <SortableSectionItem key={id} section={section} productTypeId={productTypeId} manufacturerId={manufacturerId} onUpdated={handleUpdated} />
                  ) : null
                })}
              </div>
            </SortableContext>
          ...
          {addingSection && (
            <AddForm onAdd={handleAddSection} onCancel={() => setAddingSection(false)} placeholder="Bereichsname eingeben..." />
          )}
